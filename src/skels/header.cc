...
header_gen_class::generate_header(ostream &stream, unsigned int indent)
{
  ...
  stream << "@";
  stream << "brief whether to check that all required options were provided (default 0) */";
  stream << "\n";
  stream << indent_str;
  ...
  stream << "int ";
  generate_string (parser_name, stream, indent + indent_str.length ());
  stream << "_ext (int argc, char * const *argv,";
  stream << "\n";
  stream << indent_str;
  stream << "  struct ";
  generate_string (args_info, stream, indent + indent_str.length ());
  ...
  stream << "\n";
  stream << indent_str;
  stream << " * all its fields to 0";
  stream << "\n";
  stream << indent_str;
  stream << " * ";
  stream << "@";
  stream << "return the initialized ";
  generate_string (parser_name, stream, indent + indent_str.length ());
